...
public class UI_ChoiceStage : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[0]) return; checkSign[0].SetActive(true); checkSign[0].GetComponent<Animator>().Play("Check"); };
        evt = checkBoxs[1].GetComponent<UI_EventHandler>();
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[1] || clearStageIndex + 1 < 1) return; checkSign[1].SetActive(true); checkSign[1].GetComponent<Animator>().Play("Check"); };
        evt = checkBoxs[2].GetComponent<UI_EventHandler>();
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[2] || clearStageIndex + 1 < 2) return; checkSign[2].SetActive(true); checkSign[2].GetComponent<Animator>().Play("Check"); };
        evt = checkBoxs[3].GetComponent<UI_EventHandler>();
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[3] || clearStageIndex + 1 < 3) return; checkSign[3].SetActive(true); checkSign[3].GetComponent<Animator>().Play("Check"); };
        evt = checkBoxs[4].GetComponent<UI_EventHandler>();
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[4] || clearStageIndex + 1 < 4) return; checkSign[4].SetActive(true); checkSign[4].GetComponent<Animator>().Play("Check"); };
        evt = checkBoxs[5].GetComponent<UI_EventHandler>();
        evt.OnClick += (PointerEventData p) => { if (GameManager.Instance.isClear[5] || clearStageIndex + 1 < 5) return; checkSign[5].SetActive(true); checkSign[5].GetComponent<Animator>().Play("Check"); };
